...
def hyp3_process(cfg, n):
    try:
        ...
        log.exception('proclib processing failed!')
        log.exception('Notifying user')
        hyp3proclib.failure(cfg, str(e))

    hyp3proclib.file_system.cleanup_workdir(cfg)

    log.info('proclib done')


def main():
    """
    Main entrypoint for hyp3proclib
    """
    processor = Processor(
        'proclib', hyp3_process, sci_version=hyp3proclib.__version__
    )
    processor.run()
